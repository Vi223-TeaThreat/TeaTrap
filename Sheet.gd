...
func _init() -> void:
	...
	# =====================================================================
	# ОДИН ЛИСТ НА ВСЁ, А НЕ ФАЙЛ НА ЧАСТЬ — её просьба 08.09.2026: «объедини
	# текстуры для мака и прошлые текстуры в один файл».
	#
	# ЧТО БЫЛО НАКАНУНЕ И ЧЕМ ОКАЗАЛОСЬ ПЛОХО. Днём раньше маковые столбцы
	# выкладывались отдельными файлами — казалось удобнее, чем искать их в листе
	# из семнадцати столбцов. Вышло наоборот: рисовать пришлось бы в трёх местах,
	# держать три файла в согласии и возвращать их тремя командами. Лист один,
	# и файл на него один; а чтобы столбцы не искать глазами, есть слой клеток
	# (`moss_grid.png`) и подпись ниже.
	#
	# Возвращается тоже разом: `--hand-from=res://art/moss_base.png --part=all`.
	...
	print("Столбцы слева направо: ", Plants.KINDS,
		...
		", лист мака ×", Plants.POPPY_LEAF_KINDS, ", ЛЕПЕСТОК мака ×",
		Plants.POPPY_PETAL_KINDS, " (цветок складывается из шести таких)")
	plants.free()
	quit()
