...
func stroke_at(point: Vector3, radius: float, amount: float,
		...
		for dy in range(-span, span + 1):
			for dz in range(-span, span + 1):
				...
				for j in _seed_hash[key]:
					...
					var w: float = sqrt(1.0 - smoothstep(0.0, 1.0, d)) * 0.616
					edits[j] = float(edits.get(j, 0.0)) + amount * w
	...
	var seen: Dictionary = _grown(zone)
	for j in seen:
		_refresh_cavity(j)
...
const LEDGE_AMP: float = 0.72     # насколько тянет к пласту
...
func _ledge(p: Vector3, s: float) -> float:
	...
	# Переход между полками занимает 80% высоты пласта — это четыре ячейки,
	# ровно тот порог, ниже которого решётка форму уже не держит. См. `LEDGE_AMP`.
	var stair: float = base + smoothstep(0.10, 0.90, frac)
