...
var _node_xyz: PackedInt32Array = PackedInt32Array()     # тот же узел целыми, см. `_build_nodes`
...
# НАСКОЛЬКО ЯЧЕЙКУ ПОДНЯЛИ МАЗКАМИ — МАССИВОМ, А НЕ СЛОВАРЁМ.
#
# Словарь был удобен тем, что помнил только тронутое: правок в мире тысячи, а
# ячеек триста тысяч. Но платит за это ЖИВОЙ МАЗОК, и платит дорого: растушёвка
# спрашивает правку у ячейки и у шестерых её соседей, то есть семь хеширований
# на ячейку, а ячеек в широком мазке две с половиной тысячи. Вместе с полем и
# выбросом мелочи выходило под тридцать тысяч обращений к словарю на один мазок.
#
# Массив на все семена стоит два с половиной мегабайта — против той же памяти у
# словаря на несколько тысяч записей разница невелика, а чтение стало прямым.
# В снимок сада по-прежнему уходит РЕДКАЯ запись (`edits_saved`): формат файла
# от этого не поехал, и старые сохранения читаются как читались.
var edit_val: PackedFloat64Array = PackedFloat64Array()

# ЧЕГО СТОИТ МАЗОК ПО ЧАСТЯМ. Мазок — не одно дело, а пять, и живут они в разных
# местах: обход семян кладёт массу, а всё остальное разбирается с последствиями
# по округе. Одним числом их не различить, и чинить пришлось бы наугад. Считает
# стенд отклика (`--dabbench`).
var dab_seeds_ms: float = 0.0    # обход семян: сама прибавка и каменистость
var dab_relax_ms: float = 0.0    # растушёвка по выпуклости
var dab_stone_ms: float = 0.0    # обёртка складок и швы между глыбами
var dab_fill_ms: float = 0.0     # поле и огранка
var dab_look_ms: float = 0.0     # впадина, разглаживание, затенение стыка
var dab_seeds_seen: int = 0      # семян перебрано
var dab_seeds_hit: int = 0       # и скольким досталась правка
...
var _land_fold: FastNoiseLite
var _land_grain: FastNoiseLite
...
# ВДВОЕ РЕЖЕ ПРЕЖНЕГО — её решение 05.09.2026: «сделай в 2 раза реже паттерн
# крупных трещин». Растяжка 2.6 → 5.2, то есть шаг семейств с 8.6/8.8/10.4 м
# уходит на 17/18/21 м: на глыбу в шесть с половиной метров приходится уже не
# две-три трещины, а одна, и то не всякой глыбе.
#
# Ряд выше показывает, куда это ведёт, и опасности тут нет: чем реже трещины,
# тем БОЛЬШЕ плоских граней (тело камня между ними уцелевает целиком) и тем
# меньше шипов. Что убывает — тень в ямах, и убывает до крох.
var joint_span: float = 7.6
...
func generate(radius: float, top: float, bottom: float, headroom: float,
	...
	# ПОЛЕ ПОДВЁРНУТО — этим гряды и связываются между собой (её просьба
	# 05.09.2026: «сделай генерируемые элементы ландшафта более крупными и
	# взаимосвязанными»).
	#
	# Ровный шум даёт формы, сидящие обособленно: каждая сама по себе, соседей не
	# знает. Подворот сдвигает точку, ПРЕЖДЕ ЧЕМ спросить у неё высоту, и сдвиг
	# этот сам берётся из шума — крупного и медленного. Формы от этого
	# вытягиваются вдоль общего течения, перетекают друг в друга и срастаются:
	# гряда идёт не прямой, а изгибается, ветвится и снова сходится с соседней.
	#
	# РАЗМАХ МЕНЬШЕ ДЛИНЫ ВОЛНЫ, и это не вкус: подверни поле дальше, чем на
	# половину волны, — и оно начнёт складываться само на себя, а сложенное поле
	# даёт петли и самопересечения там, где решётке нужен гладкий склон.
	_land_hill.domain_warp_enabled = true
	_land_hill.domain_warp_type = FastNoiseLite.DOMAIN_WARP_SIMPLEX
	_land_hill.domain_warp_amplitude = WARP_SWING * _land_k
	_land_hill.domain_warp_frequency = WARP_WAVE / _land_k
	# СКЛАДКИ И ЗЕРНО — разбор у постоянных `FOLD_*` и `GRAIN_*`.
	#
	# ЧАСТОТА У НИХ БЕЗ `_land_k`, И ЭТО ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ТАК. Все прочие
	# волны растягиваются вместе с островом, чтобы крутизна склона от его размера
	# не зависела. Здесь мерка другая: мелкое упирается не в остров, а в РЕШЁТКУ,
	# а ячейка 0.67 м одна и та же на любом острове. Растяни складки вместе с
	# миром — и на широком острове они уехали бы туда же, где уже стоит зыбь, то
	# есть в пустоту между решёткой и зыбью не легло бы ничего. Крутизна при этом
	# всё равно от размера не зависит: у складок не растягивается НИ высота, НИ
	# длина волны, а склон — их отношение.
	_land_fold = FastNoiseLite.new()
	_land_fold.seed = grid_seed + 4519
	_land_fold.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH
	_land_fold.frequency = FOLD_WAVE
	# ПОДВОРОТ У СКЛАДОК ТЕЧЕНИЕМ ОСТРОВА, а не своим. Складки должны ложиться
	# ВДОЛЬ крупных форм, иначе мелкий рисунок живёт сам по себе и спорит с
	# рельефом — ровно то, от чего уходили подворотом у холмов.
	_land_fold.domain_warp_enabled = true
	_land_fold.domain_warp_type = FastNoiseLite.DOMAIN_WARP_SIMPLEX
	_land_fold.domain_warp_amplitude = WARP_SWING * _land_k
	_land_fold.domain_warp_frequency = WARP_WAVE / _land_k
	# У ЗЕРНА ПОДВОРОТА НЕТ: на шести метрах течение в сто сорок его только
	# сдвигает целиком, а стоит лишнего просмотра шума на каждом семени.
	_land_grain = FastNoiseLite.new()
	_land_grain.seed = grid_seed + 6971
	_land_grain.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH
	_land_grain.frequency = GRAIN_WAVE
...
# УЗЕЛ РЕШЁТКИ ТОЖЕ РАЗ И НАВСЕГДА. Семя всю жизнь сидит в своём узле, а
# `node_of` каждый раз делил вектор на шаг и округлял три числа. Зовут его на
# всяком мазке (метка кусков), при каждой посадке и на каждом шаге роста —
# десятки тысяч раз за секунду игры. Считаем один раз при постройке мира.
func _build_nodes() -> void:
	# Метку наборов заводим ЗДЕСЬ, а не при первом мазке: иначе первый же мазок
	# в игре платил бы за раздачу памяти под триста тысяч чисел один за всех.
	_mark.resize(lattice.size())
	_mark.fill(0)
	_node_xyz.resize(lattice.size() * 3)
	for i in range(lattice.size()):
		var p: Vector3 = lattice[i] / _spacing
		_node_xyz[i * 3] = int(round(p.x))
		_node_xyz[i * 3 + 1] = int(round(p.y))
		_node_xyz[i * 3 + 2] = int(round(p.z))
...
#    УБАВЛЕНА С 1.15 ДО 0.85 ПО ЗАМЕРУ, а не на глаз. На острове вдвое шире зыбь
#    растягивается вместе с ним (±2.3 м), и камень, вставший на такой волне,
#    ломается: стенд камня показал шип круче 90° на шве между глыбами — ровно та
#    угловатость, на которую она показала кадром. При 0.85 шипов ноль, а луг
#    столом не становится: на широком острове это всё ещё ±1.7 м.
const SWELL: float = 0.85         # ±м
const SWELL_WAVE: float = 0.055   # 18 м — было 11.8
...
# ПОТОЛОК ВЫСОТЫ, ЗАМЕРЕННЫЙ 02.09.2026, И ОН ЖИВ. Выше 3.4 м решётка при
# ячейке 0.67 м рельеф не держит: замер на эталонном острове — при 3.4 м
# поверхность замкнута начисто, при 4.0 м три незамкнутых места, при 4.8 м шесть.
# Ни обрывы, ни овраги тут ни при чём: с выключенными выходило столько же.
# Гряда (`RIDGE_RISE`) стоит заметно ниже этого потолка и потому в него не
# упирается — но подниматься выше нельзя и ей.
const HILL_RISE: float = 3.4      # м — потолок высоты рельефа
# ВОЛНА УКОРОЧЕНА С 32 ДО 21 М — и это не «мельче», а наоборот.
#
# Прежде она была ШИРЕ САМОГО ОСТРОВА (32 м при 52 м поперёк), то есть на весь
# мир приходился один-полтора купола: крупная форма была ровно одна, мелкая
# зыбь слишком слаба, а на промежутке 8–20 м не было ничего. На её кадре
# 05.09.2026 это и вышло гладким блином.
#
# Теперь на острове умещается два-три ГРЕБНЯ (см. ниже), и промежуток занят ими.
const HILL_WAVE: float = 0.031    # 32 м — шире острова

# 2б. ГРЯДЫ ВМЕСТО КУПОЛОВ — её решение 05.09.2026 по рисунку поверх кадра:
#     «более крупными и взаимосвязанными», красным обведены длинные плавные дуги.
#     Из трёх предложенных путей (гряды с ложбинами / широкие валы / гряды с
#     террасами) она выбрала первый.
#
#     ЧЕМ ГРЯДА ОТЛИЧАЕТСЯ ОТ ХОЛМА. Холм — это купол: высота падает от середины
#     во все стороны, и соседний холм ему не родня. Гряда — ЛИНИЯ: высота падает
#     только вбок, а вдоль гребня держится. Линия в шуме одна — та, где он
#     переходит через ноль (тем же приёмом сделаны овраги, только они режут
#     вниз, а гряда поднимает вверх). Гребни от этого ветвятся и сходятся сами
#     собой, а между ними остаются вытянутые ложбины.
#
#     ГРЕБЕНЬ ОБЯЗАН БЫТЬ СКРУГЛЁН. Модуль шума даёт на месте гребня ИЗЛОМ, а
#     излом решётка держит хуже крутизны — это уже записано у подошвы холма
#     (`HILL_KNEE`) и стоило семи незамкнутых мест. Скругляем не отдельной
#     веткой, а самой формулой: `sqrt(n² + r²) − r` — тот же модуль, но с дугой
#     радиуса `r` на месте ребра. Ветки нет, значит и новому излому взяться
#     неоткуда.
#
#     ШИРИНА ГРЯДЫ — доля волны, которую занимает подъём. Больше — гряды
#     срастаются в сплошное плато и ложбин не остаётся; меньше — вместо гряд
#     узкие валики, которые решётка рубит на треугольники.
const RIDGE_ROUND: float = 0.18   # скругление гребня, в долях шума
const RIDGE_WIDE: float = 0.58    # какую долю волны занимает гряда
# ГРЯДА НИЖЕ КУПОЛА, И ЭТО НЕ УСТУПКА, А АРИФМЕТИКА. У купола склон — вся
# половина волны (10 м), у гряды — только её бок, вчетверо короче. Оставь
# прежние 3.4 м, и бок встанет под шестьдесят градусов: замерено — обрывов
# 22.2% против нормы в 7%, поверхность рвётся в четырнадцати местах, а формы
# выходят МЕЛЬЧЕ прежних (1.64 м вширь на полметра подъёма против 3.1).
const RIDGE_SEAT: float = 0.67    # где у гряды луг: ниже — ложбина, выше — гребень
const RIDGE_RISE: float = 2.50    # м — рисунок поверх холма, а не сам холм

# 2в. ПОДВОРОТ ПОЛЯ — то, чем гряды СВЯЗЫВАЮТСЯ между собой. Разбор — у
#     `_land_hill` в постройке шума.
const WARP_SWING: float = 5.0     # на сколько метров уводит точку
const WARP_WAVE: float = 0.014    # 70 м — течение много крупнее самих гряд

# 2г. СКЛАДКИ И ЗЕРНО — мелкий рисунок земли (её решение 07.09.2026; эталон —
#     третий кадр 06.09, где мелкую проработку она клала кистью ВРУЧНУЮ).
#
#     ЧЕГО НЕ ХВАТАЛО, И ЭТО СЧИТАЕТСЯ, А НЕ КАЖЕТСЯ. Все волны рельефа
#     растягиваются вместе с островом (`_land_k` = 2 при радиусе 26 м), и в
#     подписях выше стоят НОМИНАЛЫ, а не то, что выходит на деле:
#
#     | слой | номинал | на острове |
#     |---|---|---|
#     | подворот | 70 м | 140 м |
#     | гряда-холм | 32 м | 64 м |
#     | обрывы, зыбь, кромка | 18 м | 36 м |
#     | овраги | 20 м | 40 м |
#
...
#     эта полоса пустовала, и потому земля между крупными формами читалась
#     гладкой. Ровно туда она и клала кисть руками.
#
#     ДВА СЛОЯ, А НЕ ОДИН: полоса шириной больше десятикратной, одной волной её
#     не занять. Лестница волн выходит ровной, каждая ступень примерно вдвое с
#     половиной: 140 → 64 → 36 → 14 → 9 → 2.7 (предел решётки).
#
#     ШЕСТЬ МЕТРОВ РЕШЁТКА НЕ ДЕРЖИТ, И ЭТО ЗАМЕРЕНО, А НЕ ПРИКИНУТО. Зерно
#     сперва стояло на шести метрах: девять ячеек на волну, но весь переход от
#     гребня к ложбине укладывается в четверть волны, то есть в две ячейки с
#     небольшим. Эталонное зерно дало восемь незамкнутых мест при норме ноль.
#     На девяти метрах переход выходит в три с половиной ячейки, и поверхность
#     замкнута начисто.
#
#     СЕЧЕНИЕ У ОБОИХ — ТО ЖЕ, ЧТО У ГРЯДЫ, и это не лень, а её правило
#     «земля не должна идти прыщиками» (06.09.2026). Ровный шум даёт круглые
#     шапки; скруглённый модуль даёт ЛИНИИ — складки ветвятся, сходятся и
#     вытягиваются, как борозды на лугу. Доли `WIDE` и `SEAT` взяты у гряды без
#     изменений: при том же сечении и та же доля шума приходится на гребень,
#     а значит прибавка остаётся знакопеременной и средняя высота не уезжает —
#     те самые грабли, что подняли остров с 0.22 на 2.56 м.
#
#     ВЫСОТЫ ВЗЯТЫ ПО ЗАМКНУТОСТИ НА ДЕВЯТИ ЗЁРНАХ, А НЕ ПО ОДНОМУ.
#
#     На эталонном зерне стенка стоит высоко: складка рвёт поверхность на 0.75 м,
#     зерно на 0.22. Но эталонное зерно тут ОБМАНЫВАЕТ. На 0.40/0.12 оно
#     по-прежнему чисто, а зёрна 1 и 12345, чистые без рисунка, дали по шесть
#     незамкнутых мест. Все дыры до единой — у самой кромки, в одной точке на
#     каждом зерне, и от правки к правке они перескакивают с зерна на зерно:
#     там поверхность держится на волоске при любом возмущении.
#
#     На 0.28/0.09 счёт по шести зёрнам, где есть опорный замер, выходит ЛУЧШЕ
#     прежнего: 11 незамкнутых мест против 17 без всякого рисунка (зерно 777777
#     4 → 0, 42424242 13 → 8, эталонное 0 → 0).
#
#     УРОК: судить замкнутость по эталонному зерну одному — значит мерить запас
#     там, где его больше всего. Стенка у мелкого рисунка своя на каждом зерне,
#     и брать надо самую низкую.
const FOLD_WAVE: float = 0.071    # 14 м — и на острове те же 14, см. постройку
const FOLD_ROUND: float = 0.18    # скругление гребня, в долях шума
const FOLD_WIDE: float = 0.58     # доля волны под складкой — как у гряды
const FOLD_SEAT: float = 0.67     # где у складки луг — как у гряды
const FOLD_RISE: float = 0.28     # м — на эталонном зерне рвёт на 0.75, но
                                  # на других уже на 0.40, см. разбор выше
const GRAIN_WAVE: float = 0.11    # 9 м — последняя ступень перед решёткой
const GRAIN_ROUND: float = 0.18
const GRAIN_WIDE: float = 0.58
const GRAIN_SEAT: float = 0.67
const GRAIN_RISE: float = 0.09    # м — там же 0.22 и 0.12
...
#    СТЕНКА ШИРЕ ДВУХ ЯЧЕЕК, И ЭТО НЕ ВКУС. Её кадр 02.09.2026: «проявилось
#    множество угловатых артефактов». Стенка занимала седьмую долю полосы — на
#    склоне это полметра по горизонтали, меньше ячейки решётки (0.67 м). Всё, что
#    мельче ячейки, решётка не сглаживает, а рубит на треугольники: отсюда
#    острые грани и плавники на кадре. Теперь стенка идёт две пятых полосы —
#    около двух метров по склону, то есть три ячейки, и на них есть чем скруглить.
const BENCH: float = 1.30         # высота полки, м
const BENCH_LO: float = 0.60      # с какой доли полосы начинается стенка
const BENCH_HI: float = 1.00
...
const GULLY_WIDE: float = 0.22    # шире прежнего: было 1.6 м по дну, стало 2.2
...
# НАСКОЛЬКО КРОМКА ВИЛЯЕТ — В МЕТРАХ, А НЕ В ДОЛЯХ РАДИУСА, и это замер, а не
# вкус. Прежде виляние было десятой долей радиуса; на острове вдвое шире это
# вдвое больший размах на почти той же длине волны, и кромка стала заворачивать
# круче, чем решётка держит: 26 незамкнутых мест, все в одном месте кромки. А
# держит её ячейка, которая от размера острова не зависит вовсе, — значит и
...
# СЕЧЕНИЕ ГРЯДЫ — ОДНО НА ВСЕ ТРИ СЛОЯ рельефа: гряду-холм, складки и зерно.
# Держать его в одном месте важнее, чем сэкономить вызов: сечение это ЗАКОН
# формы, и разъедься три его списка — три слоя стали бы жить по разным правилам
# молча.
#
# ГРЕБЕНЬ ИДЁТ ТАМ, ГДЕ ШУМ ПЕРЕХОДИТ ЧЕРЕЗ НОЛЬ. Линия в шуме одна, и оттого
# формы ветвятся и сходятся сами собой, а между ними остаются вытянутые ложбины.
#
# МОДУЛЬ СКРУГЛЁН ДУГОЙ, а не взят напрямую: напрямую он даёт на гребне ИЗЛОМ,
# а излом решётка держит хуже крутизны — это уже стоило семи незамкнутых мест у
# подошвы холма. `sqrt(n² + r²) − r` — тот же модуль, но с дугой радиуса `r` на
# месте ребра, и сделано это самой формулой, а не отдельной веткой: ветки нет —
# новому излому взяться неоткуда.
static func _crest(n: float, arc: float, wide: float) -> float:
	var off: float = sqrt(n * n + arc * arc) - arc
	var c: float = clampf(1.0 - off / wide, 0.0, 1.0)
	# И подошва сходит на луг мягко — та же забота, что и о гребне.
...
func _land_height(x: float, z: float, shape: FastNoiseLite) -> float:
	...
	var foot: float = smoothstep(0.0, 1.0, rim)
	# ХОЛМ САМ СТАЛ ГРЯДОЙ, а не обзавёлся грядами поверх себя.
	#
	# Её решение 05.09.2026 по рисунку поверх кадра: «более крупными и
	# взаимосвязанными», красным обведены длинные плавные дуги. Из трёх путей
	# (гряды с ложбинами / широкие валы / гряды с террасами) она выбрала первый.
	#
	# ТРИ ЗАХОДА, И ВОТ ЧТО ОНИ ПОКАЗАЛИ. Гряду НЕЛЬЗЯ ПРИБАВИТЬ к рельефу
	# отдельной волной: всякая добавленная волна короче прежних, а значит мельчит.
	# Мерка «на полметра подъёма приходится N метров вширь» шла при этом ВНИЗ
	# (2.9 → 2.14), тогда как просили ровно обратного. Гряду надо делать ИЗ САМОЙ
	# КРУПНОЙ формы, какая в мире есть, — из холма.
	#
	# Волна у холма прежняя, 32 м; меняется только ФОРМА СЕЧЕНИЯ. Был купол:
	# высота падает от середины во все стороны, и соседний купол ему не родня.
	# Стала гряда: высота падает вбок, а ВДОЛЬ ГРЕБНЯ ДЕРЖИТСЯ. Гребень идёт там,
	# где шум переходит через ноль, поэтому гряды ветвятся и сходятся сами собой,
	# а между ними остаются вытянутые ложбины — то самое, что на рисунке.
	var ridge: float = _crest(_land_hill.get_noise_2d(x, z),
		RIDGE_ROUND, RIDGE_WIDE)
	# ГРЯДА ПОДНИМАЕТ, А ЛОЖБИНА ОПУСКАЕТ, и это не украшение, а починка.
	#
	# ГРАБЛИ: сперва гряда только ПРИБАВЛЯЛА высоту, и остров от неё поднимался
	# ВЕСЬ — средняя высота ушла с 0.22 на 2.56 м. Само по себе не беда, но за ней
	# потянулись обрывы: полки (`BENCH`) режут высоту ступенями по 1.3 м, и чем
	# выше земля, тем больше ступеней она проходит. Замерено — обрывов 14.7% при
	# норме около семи.
	#
	# Отняв среднее, делаем прибавку ЗНАКОПЕРЕМЕННОЙ: по гребню земля
	# поднимается, между гребнями опускается. Средняя высота острова остаётся
	# прежней, полки режут его столько же, сколько резали, — а ложбины, которых у
	# холмов не было по построению («холмы растут ВВЕРХ, а не вниз»), наконец
	# появляются.
	# ПОТОЛОК ВЫСОТЫ СТЕРЕЖЁТ САМ КОД, а не память: `HILL_RISE` — замеренный
	# предел решётки, и подняв `RIDGE_RISE` выше него, поверхность порвёшь.
	h += (ridge - RIDGE_SEAT) * minf(RIDGE_RISE, HILL_RISE) * _land_k * foot
	# ГЛАДКУЮ ВЫСОТУ ЗАПОМИНАЕМ ДО СТУПЕНЕЙ. По ней овраг решает, где ему резать,
	# и это не мелочь: считай он по ступенчатой — глубина прыгала бы на каждой
	# стенке обрыва, и русло получало бы поперечный уступ. Замер поймал это шипами
	# на камне (стенд камня: 3 шипа круче 90°, из них два уходят вместе с
	# оврагами), а на кадре это те самые угловатые детали.
	var smooth_h: float = h
	...
	if side < GULLY_WIDE:
		# ПРОФИЛЬ ПЛАВНЫЙ, А НЕ КВАДРАТ. Квадрат давал узкое остроконечное дно и
		# перегиб у самой кромки — на её кадре 02.09.2026 это читалось плавником,
		# а не промоиной: и то и другое мельче ячейки решётки, а всё, что мельче,
		# решётка рубит на треугольники. Плавная ступень даёт корытце с пологими
		# бортами, и оба перегиба размазаны на добрую ячейку.
		var deep: float = smoothstep(0.0, 1.0, 1.0 - side / GULLY_WIDE)
		# И НАЧАЛО ОВРАГА ТОЖЕ ПЛАВНОЕ. Прямой отрезок давал перегиб там, где
		# промоина только зарождается, — а зарождается она по всей длине склона.
		var flank: float = smoothstep(0.0, 1.0, clampf(
			(smooth_h - GULLY_FROM * _land_k) / (1.6 * _land_k), 0.0, 1.0))
		h -= deep * flank * GULLY_DEEP * _land_k
	# СКЛАДКИ И ЗЕРНО КЛАДУТСЯ ПОСЛЕДНИМИ, И ПОРЯДОК ТУТ НЕ ВКУС.
	#
	# Положи их ДО обрывов — и полки (`BENCH`) порежут их ступенями наравне с
	# рельефом: каждая складка получила бы собственную стенку в 1.3 м на четырёх
	# метрах длины. Это не мелкий рисунок, а частокол; обрывов стало бы кратно
	# больше нормы, и по её же правилу от 06.09 это прямая дорога к шипам.
	#
	# Положи их ДО оврагов — и промоина считала бы глубину от складчатой высоты,
	# получая поперечный уступ на каждой складке. Ровно это уже ловилось шипами
	# на камне, оттого выше и запомнена гладкая высота (`smooth_h`).
	#
	# Последними же они ложатся ПОВЕРХ всего разом — и на склон, и на полку
	# обрыва, и на дно оврага, — то есть ведут себя как рука с кистью: та тоже
	# проходит по готовому рельефу, а не под ним.
	#
	# ПРИБАВКА ГЛАДКАЯ, а значит поверхность порвать не может: складка это не
	# ступень, у неё всюду есть наклон, и к тому, что под ней, она может лишь
	# добавить градусов — оттого высоты и держатся много ниже крутизны склона.
	var fine: float = (_crest(_land_fold.get_noise_2d(x, z),
		FOLD_ROUND, FOLD_WIDE) - FOLD_SEAT) * FOLD_RISE
	fine += (_crest(_land_grain.get_noise_2d(x, z),
		GRAIN_ROUND, GRAIN_WIDE) - GRAIN_SEAT) * GRAIN_RISE
	# У КРОМКИ МЕЛКОГО НЕТ, И ПОЛОСА ЗАТУХАНИЯ У НЕГО СВОЯ — ВТРОЕ ШИРЕ, ЧЕМ У
	# ХОЛМА. Кромка самое тонкое место в мире: там сходятся очертание острова,
	# обрыв берега и предел поля семян.
	#
	# ЗАМЕР, А НЕ ОСТОРОЖНОСТЬ. Со складками зерно 1 давало пять незамкнутых мест,
	# и ВСЕ ПЯТЬ стояли в одной точке — в 21 метре от середины при радиусе 26, то
	# есть ровно там, где затухание холма (полоса 0.22 радиуса) только начинает
	# работать и гасит всего на пять сотых. Земля там уже валится к воде, и
	# лишняя складка её дорывает.
	#
	# ГАСИМ ДОЛЕЙ ХОЛМА, ВЗЯТОЙ ДВАЖДЫ. Своя, втрое более широкая полоса пробована
	# и отвергнута замером: она убавляла рисунок и на лугу, а дыр от этого стало
	# БОЛЬШЕ, а не меньше (эталонное зерно 0 → 5). Двойная доля не трогает середину
	# острова вовсе — там она равна единице, как и одинарная, — а в береговой
	# полосе гасит вдвое круче.
	h += fine * foot * foot
	# МЯГКИЙ ПОЛ, И СКРУГЛЕНИЕ У НЕГО СОРАЗМЕРНО ОСТРОВУ. Сам пол не растягиваем:
	# подошва острова стоит на −3.5 м при любом его размере, и опусти пол вдвое —
	# земля встретилась бы с исподом. А вот СКРУГЛЕНИЕ у перехода растянуть надо,
	# иначе на широком острове оно выходит вдвое короче ячейки и даёт по всей
	# линии перехода жёсткий перегиб — из тех самых угловатых, на которые она
	# показала кадром 02.09.2026.
	return -_smin(-h, -LAND_LOW, 0.5 * _land_k)
	...
	for i in range(seeds.size()):
		...
		if not column.has(key):
			...
			column[key] = Vector2(_land_height(g.x, g.z, shape),
				radius + EDGE_SWING * _land_edge.get_noise_2d(g.x, g.z))
...
# НАБОР ЯЧЕЕК — СПИСКОМ И МЕТКОЙ, А НЕ СЛОВАРЁМ.
#
# Мазок строит три вложенных набора: задетые ячейки, они же с кольцом соседей и
# ещё с одним кольцом. У широкой кисти это 1.7, 2.6 и 4 тысячи номеров, и каждый
# набор был словарём: `duplicate()` копировал предыдущий целиком, а всякая
# проверка «уже есть?» хешировала число. За мазок таких проверок под сотню тысяч.
#
# Место номеру — в массиве. Порядок держит список, принадлежность — метка в
# `_mark`: у каждого набора она своя и растёт, так что прежние отметки гаснут
# сами и чистить массив не приходится вовсе.
#
# ПОРЯДОК ОБХОДА ТОТ ЖЕ, ЧТО БЫЛ: словарь в Godot помнит, в каком порядке в него
# клали, и `_grown` сперва копировал набор, а потом дописывал соседей — ровно то
# же делают два прохода ниже. Числа от перестановки не поехали бы (всё, что
# считается здесь, от порядка не зависит), но сравнивать прогоны проще, когда
# менять нечего.
var _mark: PackedInt32Array = PackedInt32Array()
var _mark_stamp: int = 0

# ОБЛИК ДОГОНЯЕТ ФОРМУ (её решение 05.09.2026: «отложить тень»).
#
# Впадина и наклон света — это ОБЛИК, а не форма: где проходит земля и куда
# целится курсор, от них не зависит вовсе. А стоили они 17 миллисекунд из 42 у
# широкого мазка, и платил их тот самый кадр, в котором рука нажала.
#
# Теперь мазок только ЗАПОМИНАЕТ свою округу, а считается она в ближайшем кадре
# без мазка. При удержании кисть бьёт раз в восемь кадров — пустых кадров вдоволь.
#
# ОЧЕРЕДЬЮ МАЗКОВ, А НЕ ОДНИМ НАБОРОМ, и это стоило одной переделки. Сперва я
# копил округу всех накопившихся мазков в ОДИН набор — казалось, что это то же
# самое без повторов: `cavity` считается от поля начисто, прежнее значение не
# копится. Разглаживание, однако, читает впадину СОСЕДЕЙ, и у слитого набора
# соседи лежат внутри него самого — неразглаженные, — а у каждого мазка порознь
# они снаружи и уже разглажены. Мир от этого поехал: лиана в самопроверке дала
# 481 звено вместо 669 (растения смотрят на впадину при росте).
#
# Разброс у лианы и так огромный (26–899 звеньев по устройству), то есть это был
# не сбой, а другой бросок костей. Но правка, которая двигает мир, должна двигать
# его НАРОЧНО. Здесь нарочно откладывается ВРЕМЯ, а не итог: каждый мазок
# досчитывается своей округой, ровно как считался бы на месте.
#
# ЧТО ПРИ ЭТОМ ВИДНО ГЛАЗОМ: форма идёт за рукой как шла, а тень в щелях догоняет
# её с задержкой в кадр-другой. Ради этого всё и делалось.
# И САМ ОБЛИК ТОЖЕ ИДЁТ С ЗАПАСОМ НА КАДР, СТУПЕНЯМИ. Просто отложить его было
# мало: 17 миллисекунд, снятые с кадра мазка, целиком падали на следующий, и
# вместо одного рывка в 43 мс выходило два по 25. Считаем его тремя ступенями,
# и в каждой кадр делает столько, сколько успевает:
#
#   1. ВПАДИНА у каждой ячейки набора — считается только по полю, поэтому режется
#      по кадрам как угодно;
#   2. РАЗГЛАЖИВАНИЕ впадины — ЦЕЛИКОМ, не по частям. Оно читает впадину соседей
#      и тут же её переписывает: разрежь его по кадрам — и на стыке кусков ляжет
#      шов, который потом ничем не исправить. Ступень эта дешёвая (около 4 мс);
#   3. НАКЛОН СВЕТА по кольцу вокруг набора — снова только чтение готового, снова
#      режется свободно.
#
# ЧТО НАКОПИЛОСЬ, ПОКА СЧИТАЛИ, ждёт своей очереди отдельным набором: подмешать
# новые ячейки в тот, что уже на второй ступени, значило бы разгладить их по
# несчитанной впадине.
var _look_queue: Array = []                              # округи мазков, ждущие своей очереди
var _look_wait: PackedInt32Array = PackedInt32Array()    # та, что досчитывается сейчас
var _look_ring: PackedInt32Array = PackedInt32Array()    # кольцо под наклон света
var _look_step: int = 0                                  # 0 нечего, 1 впадина, 2 разглаживание, 3 наклон
var _look_at: int = 0                                    # докуда дошли в нынешней ступени


func look_pending() -> bool:
	return _look_step != 0 or not _look_queue.is_empty()


# Досчитать отложенный облик, сколько влезет в запас. Ноль запаса — досчитать всё.
func flush_look(budget: float = 0.0) -> void:
	var deadline: int = Time.get_ticks_usec() + int(budget * 1000.0)
	while true:
		if _look_step == 0:
			if _look_queue.is_empty():
				return
			_look_wait = _look_queue.pop_front()
			_look_step = 1
			_look_at = 0
		if _look_step == 1:
			while _look_at < _look_wait.size():
				_refresh_cavity(_look_wait[_look_at])
				_look_at += 1
				if budget > 0.0 and Time.get_ticks_usec() >= deadline:
					return
			_look_step = 2
		if _look_step == 2:
			_smooth_cavity(_look_wait)
			_look_ring = _ring_of(_look_wait)
			_look_step = 3
			_look_at = 0
			if budget > 0.0 and Time.get_ticks_usec() >= deadline:
				return
...
func bulge_at(index: int, from_edits: bool = false) -> float:
	var here: Vector3 = seeds[index]
	var f0: float = edit_val[index] if from_edits else fill[index]
	var raw := Vector3.ZERO
	...
	for k in range(6):
		...
		if len2 < 0.000001:
			continue
		var df: float = (edit_val[s] if from_edits else fill[s]) - f0
		raw += d * (df / len2)
		away += d
		sum += df
		count += 1
...
func stroke_at(point: Vector3, radius: float, amount: float,
		stone_push: float = 0.0) -> PackedInt32Array:
	var _t_phase: int = Time.get_ticks_usec()
	# Семя лежит в хеше ровно в одной клетке, а клетки перебираются по разу —
	# значит, дважды одно и то же сюда не попадёт, и проверять нечего.
	...
	# КИСТЬ — ШАР, А ПЕРЕБИРАЛСЯ КУБ. Обход идёт по клеткам хеша, и куб со
	# стороной в два радиуса с запасом больше вписанного шара впятеро с лишним:
	# у широкой кисти это 14 м поперёк против 9.8 м самой кисти. Каждое лишнее
	# семя всё равно доходило до `length()` и деления, чтобы тут же отсеяться.
	#
	# Отсекаем ДВАЖДЫ И ПО КВАДРАТУ РАССТОЯНИЯ, без корня: сперва целую клетку
	# хеша по ближайшему её углу, потом само семя. Обе проверки ТОЧНЫЕ, а не
	# приблизительные: дальше `radius * stroke_reach` мазок не кладёт ничего ни
	# при каких условиях — у глыбы своя мерка (`_block_dist`) только УВЕЛИЧИВАЕТ
	# расстояние, обрезая выступающие вершины кругом кисти, и уменьшить его не
	# может. Значит, ни одно отсеянное семя не получило бы правки.
	var reach: float = radius * minf(1.0, stroke_reach)
	...
	for dx in range(-span, span + 1):
		for dy in range(-span, span + 1):
			for dz in range(-span, span + 1):
				...
				if not _seed_hash.has(key):
					continue
				# Ближайшая точка клетки к середине мазка: клетка целиком за
				# кистью — значит, и все её семена тоже.
				var corner: Vector3 = Vector3(key) * _cell_size
				var near := Vector3(
					clampf(point.x, corner.x, corner.x + _cell_size),
					clampf(point.y, corner.y, corner.y + _cell_size),
					clampf(point.z, corner.z, corner.z + _cell_size))
				if near.distance_squared_to(point) >= reach2:
					continue
				dab_seeds_seen += _seed_hash[key].size()
				for j in _seed_hash[key]:
					var off: Vector3 = seeds[j] - point
					if off.length_squared() >= reach2:
						continue
					...
					if not cut.is_empty():
						...
						#
						# СРЕЗ МЯГКИЙ, А НЕ ЖЁСТКИЙ (`block_clip`). Жёсткий
						# максимум двух тел даёт ребро по всей линии, где они
						# сравнялись, — а это и есть кромка мазка, на которой
						# сидят шипы вне швов. Внутри глыбы ту же болезнь лечили
						# тем же лекарством (`block_round` в `_block_dist`).
						d = maxf(_block_dist(off, cut), d) if block_clip <= 0.0 \
							else -_smin(-_block_dist(off, cut), -d, block_clip)
					if d >= 1.0:
						continue
	...
	dab_seeds_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
	dab_seeds_hit += touched.size()
	...
	var zone: PackedInt32Array = _ring_of(touched)
	...
	# В ДВА ПРОХОДА, И ЭТО ОБЯЗАТЕЛЬНО: выпуклость смотрит на правки СОСЕДЕЙ, и
	# запиши мы посчитанное сразу — соседи считались бы уже по новому.
	# Промежуток держим массивом чисел, а не словарём, и числа в нём полной
	# точности: `edits` — счёт в двойной, и укоротить его значило бы сдвинуть
	# форму насыпи на последних цифрах.
	var mixed := PackedFloat64Array()
	mixed.resize(zone.size())
	var at_z: int = 0
	for j in zone:
		...
		mixed[at_z] = edit_val[j] + bulge_at(j, true) * soft
		at_z += 1
	at_z = 0
	for j in zone:
		edit_val[j] = mixed[at_z]
		at_z += 1

	dab_relax_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
	_t_phase = Time.get_ticks_usec()
	...
	var edge: PackedInt32Array = _ring_of(zone)
	for j in edge:
		_refresh_seam(j)

	dab_stone_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
	_t_phase = Time.get_ticks_usec()

	for j in zone:
		if absf(edit_val[j]) < 0.002:
			edit_val[j] = 0.0
	...
	var seen: PackedInt32Array = edge
	dab_fill_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
	_t_phase = Time.get_ticks_usec()
	if _batching:
		...
		for j in seen:
			_batch[j] = true
		dab_look_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
		return zone
	# ОБЛИК ОТКЛАДЫВАЕМ — см. `_look_queue`. Считать его тут значит платить за него
	# тем самым кадром, в котором рука нажала. Округу кладём в очередь ЦЕЛИКОМ и
	# отдельно от чужих: досчитается она ровно так, как посчиталась бы здесь.
	_look_queue.append(seen)
	dab_look_ms += float(Time.get_ticks_usec() - _t_phase) / 1000.0
	return zone
...
func restore_state(ed: Dictionary, st: Dictionary, lu: Array) -> Array:
	# Снимок хранит РЕДКУЮ запись — только тронутые ячейки. Раскладываем её по
	# массиву, начисто стирая всё, что могло остаться от прежнего мира.
	edit_val.resize(seeds.size())
	edit_val.fill(0.0)
	for j in ed:
		edit_val[int(j)] = float(ed[j])
	stone = st.duplicate()
...
# Насколько трещину уводит волна, в метрах. Меньше глыбы нарочно: внутри одного
# камня трещина должна идти почти прямой, а гулять — от глыбы к глыбе.
const JOINT_SWAY: float = 3.2
# СНОС НАПРАВЛЕНИЯ — вторая волна, много длиннее первой (её слово 07.09.2026:
# «сделай направление трещин более хаотичным»).
#
# ЧЕМ ОН ОТЛИЧАЕТСЯ ОТ ВОЛНЫ ВЫШЕ, И ПОЧЕМУ ЭТО ДВЕ РАЗНЫЕ ВЕЩИ. Уводя трещину,
# волна делает две работы сразу: НАКЛОНЯЕТ плоскость (это первая производная) и
# ГНЁТ её (вторая). Наклон — как раз то, что просят: у этого камня трещины идут
# так, у соседнего иначе. Изгиб же внутри одной глыбы — то, что 05.09.2026
# вылезло шипом: две дуги сходятся под острым углом.
#
# Разделить их можно только длиной волны. Наклон идёт как размах, делённый на
# четверть волны; изгиб — как он же, делённый на квадрат. Значит длинная волна
# даёт много наклона и почти ноль изгиба, и её размах можно брать смело.
#
# ДЛИНУ ВОЛНЫ ПРИШЛОСЬ УДВОИТЬ ПО ЗАМЕРУ. Сперва стояло 39 м при размахе 4 м —
# наклон до 22°, и на пробном массиве худшее вогнутое ребро ушло с 75.1° на
# 87.3° при пороге 90. Шипов ещё не было, но запас в три градуса — это не запас;
# ровно так 07.09.2026 и вылезли шипы на настоящем острове, где камень сложнее
# пробного.
#
# Замер по размаху при волне 39 м: 0 м — 75.1°, 2 м — 84.1°, 3 м — 81.1°,
# 4 м — 87.3°. Число скачет (худшее ребро — это максимум по многим), но ход
# ясен. Волна 66 м при том же размахе 4 м даёт 74.8°, то есть запас как без
# сноса вовсе, а направление всё так же гуляет — на 14° по острову.
#
# Своё смещение у каждого семейства (`off`), иначе все три поедут заодно и
# решётка трещин останется прежней, только косой.
const JOINT_DRIFT: float = 4.0    # размах сноса, м
const JOINT_DRIFT_WAVE: float = 0.18   # доля частоты волны выше: 11.8 / 0.18 = 66 м
...
# УБАВЛЕНА С 2.2 ДО 1.4 (её кадр 07.09.2026: «вылезли шипы. Очень скверный
# признак»). С тех пор как трещины стали вдвое реже, каждая осталась одна и
# режет в одиночку — прежняя глубина у одинокой трещины выворачивает поверхность.
# Замер на широком массиве: при 2.2 худшее ребро 84.3° при пороге 90 — запаса
# почти нет; при 1.4 оно 69.6°, а плоских граней даже больше (46.5% против 44.1).
var crack_deep: float = 1.4
...
# Насколько мягко круг кисти срезает выступающие части глыбы, долей радиуса
# кисти. Ноль — жёсткий максимум, дающий ребро по линии среза. См. `stroke_at`.
var block_clip: float = 0.10
...
func _build_joints(grid_seed: int) -> void:
	...
	# УГОЛ МЕЖДУ ОТВЕСНЫМИ СЕМЕЙСТВАМИ — СВОЙ У КАЖДОГО МИРА, а не один на все
	# (её слово 07.09.2026 про хаотичность). Пока он стоял ровно на 75°, глыбы
	# во всех мирах выходили одним и тем же ромбом; теперь ромб у каждого острова
	# свой. Прямого угла по-прежнему избегаем — он даёт кирпичи.
	var skew: float = deg_to_rad(rng.randf_range(62.0, 88.0))
...
func _joints(p: Vector3, s: float) -> float:
	...
	for js in _joint_sets:
		...
		# РАЗМАХ ВОЛНЫ — В МЕТРАХ, А НЕ В ДОЛЯХ ШАГА, и это не придирка.
		#
		# ГРАБЛИ 05.09.2026. Стояло `step * 0.35`, и пока шаг семейств был 8.6–10.4
		# м, размах выходил около трёх метров — меньше глыбы, трещина шла по ней
		# почти прямой. Стоило по её просьбе развести семейства вдвое (17–21 м),
		# как размах стал шестиметровым: трещина начала выписывать дугу ВНУТРИ
		# одной глыбы, две таких дуги сходились под острым углом, и на стыке
		# вылезал шип — замерено, вогнутое ребро 103° при норме до 90.
		#
		# Размах трещины — свойство ПОРОДЫ, а не того, часто ли трещины идут.
		# Значит, и мерка ему своя, в метрах.
		# СНОС НАПРАВЛЕНИЯ — вторая волна, втрое длиннее. Разбор у `JOINT_DRIFT`:
		# длинная волна наклоняет плоскость, но почти не гнёт её, и потому её
		# размах можно брать смело там, где короткой нельзя.
		var dw: float = JOINT_DRIFT_WAVE
		var lift: float = _ledge_warp.get_noise_3d(p.x + off, p.y + off, p.z + off) \
			* JOINT_SWAY \
			+ _ledge_warp.get_noise_3d((p.x + off * 3.0) * dw,
				(p.y + off * 3.0) * dw, (p.z + off * 3.0) * dw) * JOINT_DRIFT
...
# ПРАВКИ ДЛЯ СНИМКА САДА — редкой записью, как и были. В снимок идёт только
# тронутое, а нетронутого в мире подавляющее большинство: писать триста тысяч
# нулей значило бы раздуть файл на пустом месте и сломать старые сохранения.
func edits_saved() -> Dictionary:
	var out: Dictionary = {}
