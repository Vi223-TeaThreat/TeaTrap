		...
		for dy in range(-span, span + 1):
			for dz in range(-span, span + 1):
				...
				for j in _seed_hash[key]:
					...
					if amount > 0.0:
						# Каменистость растекается ШИРЕ, чем сама прибавка
						# высоты: иначе камнем оказывается только сердцевина
						# мазка, и глыба читается белым пятачком вместо тела.
						stone[j] = clampf(float(stone.get(j, 0.0))
							+ (stone_amount * 2.0 - 1.0) * amount * w * 1.4, 0.0, 1.0)
					touched[j] = true
