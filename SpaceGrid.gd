...
func generate(radius: float, top: float, bottom: float, headroom: float,
	...
	# ГРЯДЫ — своя волна и свой подворот. Подворот у них СИЛЬНЕЕ, чем у холмов:
	# гряда и должна вилять, ветвиться и сходиться с соседней, а холму хватает
	# того, чтобы вытянуться и срастись с соседним.
	_land_ridge = FastNoiseLite.new()
	_land_ridge.seed = grid_seed + 4519
	_land_ridge.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH
	_land_ridge.frequency = RIDGE_WAVE / _land_k
	_land_ridge.domain_warp_enabled = true
	_land_ridge.domain_warp_type = FastNoiseLite.DOMAIN_WARP_SIMPLEX
	_land_ridge.domain_warp_amplitude = WARP_SWING * 1.6 * _land_k
	_land_ridge.domain_warp_frequency = WARP_WAVE / _land_k
...
const RIDGE_WAVE: float = 0.024   # 42 м
...
func _land_height(x: float, z: float, shape: FastNoiseLite) -> float:
	...
	var wave: float = _land_hill.get_noise_2d(x, z)
	# СКРУГЛЁННЫЙ МОДУЛЬ. Обычный дал бы на месте гребня ИЗЛОМ, а излом решётка
	# держит хуже крутизны — это уже записано выше про подошву. Здесь та же форма,
	# но с дугой радиуса `RIDGE_ROUND` на месте ребра, и сделано это самой
	# формулой, а не отдельной веткой: ветки нет — новому излому взяться неоткуда.
	var off_crest: float = sqrt(wave * wave + RIDGE_ROUND * RIDGE_ROUND) \
		- RIDGE_ROUND
	var ridge: float = clampf(1.0 - off_crest / RIDGE_WIDE, 0.0, 1.0)
	# И подошва сходит на луг мягко — та же забота, что была у купола.
