...
var crack_line: PackedFloat32Array = PackedFloat32Array()
...
# Далеко ли до ближайшей ЧЕРТЫ трещины, в долях её полутолщины. Уходит в
# покраску через вершины: тонкую длинную черту решётка нарисовать не может, а
# шейдер может. См. `crack_line`.
var _line_here: float = 1.0
# Насколько глубоко в ЯДРЕ трещины сидит ячейка, которую считают сейчас. См.
# `_joints` — там же, зачем это отдельно от глубины вычета.
var _prof_here: float = 0.0
...
func _joints(p: Vector3, s: float, body: float) -> float:
	...
	for js in _joint_sets:
		...
		# ГДЕ У ТРЕЩИНЫ ЯДРО, а где пологое плечо. Ноль на теле камня, единица на
		# самом дне — и НЕ ЗАВИСИТ от того, крупная трещина или мелкая.
		#
		# Это не то же, что глубина вычета. Вычет говорит, сколько поля отняли, и
		# у мелкой трещины он мал даже на дне; а тень должна ложиться в ядро
		# ЛЮБОЙ трещины, иначе видно будет только каждую пятую.
		#
		# И главное, ради чего он заведён: раскрыв трещины держит решётка, уже
		# метра его не сделать. Но у выветренного камня кромка и так скруглена, а
		# тёмная щель много уже всего углубления. Значит, пологое плечо оставляем
		# решётке, а тень сводим к ядру — края щели смыкаются на вид, не ломая
		# поверхности.
		_prof_here = maxf(_prof_here, prof)
		# ДАЛЕКО ЛИ ДО ЧЕРТЫ — В МЕТРАХ, а не в долях ложбины.
		#
		# ГРАБЛИ, И ОНИ ЖЕ ОБЪЯСНЯЮТ, ПОЧЕМУ ТРЕЩИН НЕ БЫЛО ВИДНО. Сперва я мерил
		# долями полутолщины — и замер показал, что под чертой оказалось 48.7%
		# камня, а в теле трещины 89.7%. То есть трещина была не чертой, а всем
		# камнем сразу: при шаге 2.3 м и полутолщине 0.7 м одно семейство
		# занимает 61% пространства, а три вместе — девяносто.
		#
		# Форма иначе не может: ложбину уже 0.7 м решётка не удержит. А КРАСКА
		# МОЖЕТ — она считается по точкам кадра и решёткой не связана. Поэтому
		# ширины у них разные: ложбина широкая и мягкая, черта в ней тонкая.
		# Меряем в метрах, чтобы толщина черты не зависела от того, какой ширины
		# вышла ложбина.
		#
		# Чем крупнее трещина, тем черта жирнее: делим на разряд.
		# ЧЕРТИМ ТОЛЬКО КРУПНЫЕ И СРЕДНИЕ. Мелкая трещина — это надлом по грани, у
		# него черты нет. Черти все подряд — и вместо раскола на камне окажется
		# сетка: замерено, под чертой было 23.7% камня вместо считаных процентов.
		if rank >= 0.5:
			_line_here = minf(_line_here, dist / (0.55 + 0.45 * rank))
